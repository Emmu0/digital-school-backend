...
module.exports = app => {
  ...
  router.post("/:id", fetchUser, [],
    async (req, res) => {
      ...
      let times = 0;
      const arry = [];
      console.log("Request files =>",arry)
      ...
      for (const f in req.files) {
        ...
        let uploadPath = process.env.FILE_UPLOAD_PATH + req.params.id;
        let filePath = uploadPath + '/' + fileRec.id + '.' + fileDetail.name.split('.')[1];
        console.log('filePath',filePath)
        console.log('newReq for File =>',newReq)
        try {
          if (fs.existsSync(uploadPath)) {
            ...
            if (times === 0) { fs.mkdirSync(uploadPath); times++; }
            fileDetail.mv(filePath, (err) => {
              if (err) {
                return res.send(err);
              }
...
router.get("/:id/download", async (req, res) => {
  try {
    ...
    const filePath = process.env.FILE_UPLOAD_PATH + parentId +"/"+ fileId + '.' + fileType;
    //let filePath = process.env.FILE_UPLOAD_PATH + parentId +"/"+ fileId;

    res.attachment(fileTitle+'.'+fileType);
    res.download(filePath, fileTitle,function(err) {
      if(err) {
  ...
  router.delete("/:id", fetchUser, async (req, res) => {
    ...
    if (resultFile) {
      ...
      //filePath = uploadPath + '/' + resultFile.id + '.' + type;
